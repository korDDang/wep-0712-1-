...
int cnt=0;
try{
	String sql="select count(*) from store0712";
...
<br>
<h2>매장 정보 목록</h2>
<div style="margin: 0 15%;">총 <%=cnt %>개의 매장정보가 있습니다.</div>
<hr>
...
int no=0;
try{
	String sql="select s.storeCode,s.storeName,s.productCode,p.productName,s.Qty from product0712 p,store0712 s where p.productCode=s.productCode order by s.storeCode";
	pstmt=conn.prepareStatement(sql);
	rs=pstmt.executeQuery();
	while(rs.next()){
